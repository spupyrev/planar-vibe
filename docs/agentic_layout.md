...
node scripts/run-dataset-multi-algorithm-batch.mjs \
  ...
  --output evaluation_data/gpt-train-results.csv
```

The script also writes `evaluation_data/gpt-train-results-scores.csv`. In that score file, `ok=0` or a missing metric value counts as `0.0` for that metric before p50.
...
1. Put the implementation in `static/js/layout-gpt.js`.

2. Export it as `window.PlanarVibeGPT.applyGPTLayout(cy, options)`.
...
4. Do use existing shared utilities freely. If a useful helper is private to another layout file, copy the needed code into `layout-gpt.js`. You can use exported public algorithms/methods as needed, but don't expose new exports.
...
Work in stages. For each stage, write the idea or goal in `docs/gpt_log.md`, prototype it, run focused experiments on `planar_train.dot`, then record whether it helped or was discarded.
